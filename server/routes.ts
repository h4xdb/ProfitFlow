...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/receipt-books", authenticateToken, requireAdminOrManager, async (req: AuthenticatedRequest, res) => {
    try {
      const receiptBookData = insertReceiptBookSchema.parse({ ...req.body, createdBy: req.user!.id });
      
      // Check if book number already exists
      const existingBook = await storage.getReceiptBookByNumber(receiptBookData.bookNumber);
      if (existingBook) {
        return res.status(400).json({ message: "Book number already exists" });
      }

      // Calculate total receipts
      receiptBookData.totalReceipts = receiptBookData.endingReceiptNumber - receiptBookData.startingReceiptNumber + 1;
